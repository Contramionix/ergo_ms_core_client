...
onMounted(async () => {
  try {
    ...
    if (!accessData.access_to_panel) {
      toast.error(tGlobal('admin.clientMonitor.noAdminAccess'))
    ...
    logError('Client monitor: ошибка проверки прав', error)
    accessDeniedState.active = true
    accessDeniedState.title = tGlobal('admin.access.deniedTitle')
    accessDeniedState.message = tGlobal('admin.clientMonitor.accessCheckFailed')
  } finally {
...
<template>
  ...
  <div v-else-if="hasAdminAccess" class="admin-page">
    <div class="page-header">
      <h1 class="page-title">{{ tGlobal('admin.clientMonitor.pageTitle') }}</h1>
      <p class="page-subtitle">{{ tGlobal('admin.clientMonitor.pageSubtitle') }}</p>
    </div>
